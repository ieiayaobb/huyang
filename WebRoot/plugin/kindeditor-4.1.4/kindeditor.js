...
function _bindNewlineEvent() {
	...
	K(doc).keydown(function(e) {
		...
		window.tagName = getAncestorTagName(self.cmd.range);
		/*
		 * add by ynshen
		 * 
		 */
		window.currentValue = self.cmd.selection().sel.focusNode.nodeValue;
		if(window.tagName == "h1" && e.which == 13){
			if(window.targetNode){
				window.targetNode.name = window.currentValue;
				zTreeObj.updateNode(window.targetNode);
				window.targetNode = null
			}else{
				var tempNodeId = "__node_" + window.count++
				zTreeObj.addNodes(null, {name:self.cmd.selection().sel.focusNode.nodeValue,id:"__node_" + window.count});
				self.cmd.selection().sel.focusNode.nodeId = "__node_" + window.count;
			}
		}
